...
void set_pwm(uint8_t yaw_duty_cycle, bool yaw_direction, bool yaw_enable, uint8_t pitch_duty_cycle, bool pitch_direction, bool pitch_enable) {
	*((uint32_t *)jiwy_map) = yaw_enable << 8 | yaw_direction << 9 | yaw_duty_cycle | pitch_enable << 24 | pitch_direction << 25 | pitch_duty_cycle << 16;
}
...
void home_yaw() {
	...
	do {
		set_pwm(20, false, true, 0, false, false);
		sleep(0.01);
		prev_enc = enc;
		get_encoders(&enc, NULL);
	} while (prev_enc-enc != 0);
	set_pwm(0, false, false, 0, false, false);
}
...
void home_pitch() {
	...
	do {
		set_pwm(0, false, false, 20, false, true);
		sleep(0.01);
		prev_enc = enc;
		get_encoders(NULL, &enc);
	} while (prev_enc-enc != 0);
...
void dance() {
	...
	while(1) {
		...
		set_pwm(20, yaw_dir, true, 20, pitch_dir, true);
		sleep(0.01);
	}
}
